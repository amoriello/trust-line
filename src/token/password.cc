...
uint8_t EncryptPassword(const Password& pass, const SymKey& sym_key,
                        uint8_t* p_data) {
  ...
  g_p_aes->cbc_encrypt((uint8_t*)&pass.data[0], pass_cipher_begin,
                       sizeof(Password::data) / block_size, iv);
  return 80;
}


uint8_t DecryptPassword(const uint8_t* p_data, const SymKey& sym_key,
                        Password* p_pass) {
